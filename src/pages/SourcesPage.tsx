...
export function SourcesPage() {
  ...
  return <section className="section-pad interior-page">
    ...
    <div className="evidence-notice"><span className="evidence-notice-icon"><Info aria-hidden="true" size={21} strokeWidth={1.8} /></span><p><strong>Evidence standard:</strong> Every claim names the publication behind it. “Refresh needed” flags a current figure, commentary, or research lead to confirm before public use.</p></div>
    ...
    <div className="source-grid">{results.map((item) => {
      ...
      return <article className="source-card" key={item.id}>
        <div className="source-card-top"><span className="category-badge">{item.category} · {item.categoryLabel}</span><span className={`verification-badge ${needsRefresh ? "needs-refresh" : ""}`}>{needsRefresh ? <RefreshCw aria-hidden="true" size={13} /> : <BadgeCheck aria-hidden="true" size={13} />}{needsRefresh ? "Refresh needed" : "Source checked"}</span></div>
        <h2>{item.title}</h2><p className="source-claim">{item.claim}</p>
        <dl>
